...
function insert_box($db, $end_date, $title, $secret_code, $id_user, $box, $id){
    $stm = $db->prepare("INSERT INTO user (date_insertion, title, access_code, user_id, box_nr, unit_id) VALUES (:end_date, :title, :secret_code, 'Texas', :id_user, :box,:id)");
    $stm->bindValue(':end_date', $end_date);
    $stm->bindValue(':title', $title);
    $stm->bindValue(':secret_code', $secret_code);
    $stm->bindValue(':id_user', $id_user);
...
if (isset($_SESSION["name"]) && isset($box) && isset($id) ){
            ...
            <form action="./edit-box.php" method="post">
                <div class="edycja">
                    ...
                    <div class="flekser"><span class="box-number"><?php echo $selected_box; ?></span>
                        ...
                        <input type="button" id="clear_fields" value="Opróżnij skrytkę"></div>
                    </div>
                    <div class="descriptive-container">
                        <div class="descriptive-container-end"><label for="title" class="">Tytuł</label></div>                
                        <div class=""><input class="" id="box_title" name="title" value="<?php echo $box_info[0]; ?>" type="text"></div>
                        <div class="descriptive-container-end"><label for="secret_code" class="">kod do skrytki</label></div> 
                        <div class=""><input class="" id="box_secret_code" name="secret_code" value="<?php echo $box_info[1]; ?>" type="number"></div>
                        <div class="descriptive_container_end"><label for="end_date" class="">data opróżnienia 1</label></div>  
                        <div class=""><input class="" id="box_end_date" name="end_date" value="<?php echo $box_info[2]; ?>" type="date"></div>
                        <div class="descriptive-container-end"><label for="id_user" class="">id czytelnika</label></div>      
